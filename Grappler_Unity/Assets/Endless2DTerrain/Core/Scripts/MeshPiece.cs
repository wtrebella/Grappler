...
namespace Endless2DTerrain
{
    public class MeshPiece
    {
        ...
        private void CreateMesh(){
            ...
            {
                renderer.sharedMaterial = settings.MainMaterial;
                renderer.sharedMaterial.renderQueue = RenderQueue.FrontPlane;
               
            }
            if (PlaneType == Plane.Detail && settings.DetailMaterial !=null)
            {
                renderer.sharedMaterial = settings.DetailMaterial;
                renderer.sharedMaterial.renderQueue = RenderQueue.DetailPlane;
              
            }
            if (PlaneType == Plane.Top && settings.DrawTopMeshRenderer && settings.TopMaterial !=null)
            {
                renderer.sharedMaterial = settings.TopMaterial;
            }
